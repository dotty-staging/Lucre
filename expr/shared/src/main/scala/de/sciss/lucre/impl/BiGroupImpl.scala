...
object BiGroupImpl {
  ...
  type LeafImpl[T <: Txn[T], A] = (SpanLike, Vec[Entry[T, A]])

  type TreeImpl[T <: Txn[T], A] = SkipOctree[T, LongPoint2DLike, LongSquare, LeafImpl[T, A]]
  ...
  private def readEntry[T <: Txn[T], A <: Elem[T]](in: DataInput, targets: Targets[T])
    ...
    val span  = SpanLikeObj.read[T](in)
    val value = Elem       .read[T](in).asInstanceOf[A]
  ...
  final def copyTree[In <: Txn[In], Out <: Txn[Out], E[~ <: Txn[~]] <: Elem[~],
    Repr <: Impl[Out, E[Out], Repr]](in: TreeImpl[In, E[In]], out: TreeImpl[Out, E[Out]], outImpl: Repr)
  ...
  abstract class Impl[T <: Txn[T], A, Repr <: BiGroup.Modifiable[T, A]]
    extends Modifiable[T, A]
      with SingleEventNode[T, BiGroup.Update[T, A, Repr]] {
...
//    type A = A
    ...
    final def treeHandle: TreeImpl[T, A] = tree

    // Note: must be after `EntryFmt`
    protected final def newTree()(implicit tx: T): TreeImpl[T, A] =
      SkipOctree.empty[T, LongPoint2DLike, LongSquare, LeafImpl[T, A]](BiGroup.MaxSquare)

    // Note: must be after `EntryFmt`
    protected final def readTree(in: DataInput)(implicit tx: T): TreeImpl[T, A] =
      SkipOctree.read[T, LongPoint2DLike, LongSquare, LeafImpl[T, A]](in)
  ...
  def newModifiable[T <: Txn[T], E[~ <: Txn[~]] <: Elem[~]](implicit tx: T): Modifiable[T, E[T]] =
    new Impl1[T, E](Targets[T]()) {
      val tree: TreeImpl[T, E[T]] = newTree()(tx)
    }

  private def read[T <: Txn[T], E[~ <: Txn[~]] <: Elem[~]](in: DataInput, _targets: Targets[T])
                                                          (implicit tx: T): Impl[T, E[T], Impl1[T, E]] =
    new Impl1[T, E](_targets) {
      val tree: TreeImpl[T, E[T]] = readTree(in)(tx)
    }

  private abstract class Impl1[T <: Txn[T], E[~ <: Txn[~]] <: Elem[~]](protected val targets: Targets[T])
    extends Impl[T, E[T], Impl1[T, E]] { in =>

    final def tpe: Obj.Type = BiGroup

    protected type GroupAux[~ <: Txn[~]] = BiGroup[~, E[~]]

    override protected def group: Impl1[T, E] = this

    override def toString: String = s"BiGroup${tree.id}"

    def modifiableOption: Option[BiGroup.Modifiable[T, E[T]]] = Some(this)

    protected def entryFormat: TFormat[T, Entry[T, E[T]]] = Entry.format

    protected def mkEntry(tgt: Targets[T], span: SpanLikeObj[T], elem: E[T])(implicit tx: T): Entry[T, E[T]] =
      new EntryImpl[T, E[T]](tgt, span, elem).connect()

    private[lucre] override def copy[Out <: Txn[Out]]()(implicit txIn: T, txOut: Out, context: Copy[T, Out]): Elem[Out] = {
      new Impl1[Out, E](Targets[Out]()) { out =>
        val tree: TreeImpl[Out, E[Out]] = out.newTree()

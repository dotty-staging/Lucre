...
object SpanExtensions  {
  ...
  private[this] object SpanTuple2s extends ExprTypeExtension1[SpanObj] {
    // final val arity = 2
    ...
    def readExtension[T <: Txn[T]](opId: Int, in: DataInput, targets: Targets[T])
      ...
      import BinaryOp._
      opId /* : @switch */ match {
        case Apply.id => Apply.read[T](in, targets)
        case Shift.id => Shift.read[T](in, targets)
  ...
  final class Tuple2[T <: Txn[T], T1, ReprT1[~ <: Txn[~]] <: Expr[~, T1], T2, ReprT2[~ <: Txn[~]] <: Expr[~, T2]](

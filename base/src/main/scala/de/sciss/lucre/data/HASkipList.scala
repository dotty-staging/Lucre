...
//
//import de.sciss.lucre.stm.{Base, Comp, Sink, Sys}
...
//
//  private final class SetSer[S <: Base.TA[Tx, Acc], Tx, Acc, A](S: S)(keyObserver: SkipList.KeyObserver[Tx, A])
//                                            (implicit ordering: Ordering[Tx, A],
//                                             keySerializer: Serializer[Tx, Acc, A])
//    extends Serializer[Tx, Acc, HASkipList.Set[S, A]] {
//
//    def read(in: DataInput, access: S.Acc)(implicit tx: Tx): HASkipList.Set[S, A] =
//      HASkipList.Set.read[S, A](S)(in, access, keyObserver)
//
...
//  }
//
////  private final class MapSer[S <: Base, A, B](keyObserver: SkipList.KeyObserver[S.Tx, A])
...
////  }
//
//  def debugFindLevel[S <: Base, A](S: S)(list: SkipList[S, A, _], key: A)(implicit tx: S.Tx): Int = {
//    // type S = list.$
//    list match {
//      case impl0: Impl[S.type, A, _] =>
//        val h = impl0.height(tx)
//
//        @tailrec
...
//  }
//
//  private final class SetImpl[S <: Base, A](val id: S#Id, val minGap: Int,
...
//  }
//
//  private final class MapImpl[S <: Base, /* @spec(KeySpec) */ A, /* @spec(ValueSpec) */ B](val id: S#Id, val minGap: Int,
//                                                                                             protected val keyObserver: SkipList.KeyObserver[S.Tx, A],
...
//  }
//
//  private sealed trait Impl[S <: Base, /* @spec(KeySpec) */ A, E]
//    extends HeadOrBranch[S, A, E] /* with Serializer[S.Tx, S.Acc, Node[S, A, E]] */ with Comp[S] /* Mutable.Impl[S] */ {
//    impl =>
//
//    // ---- abstract ----
//
...
//    }
//
//    implicit object Ser extends Serializer[S.Tx, S.Acc, Node[S, A, E]] {
...
//  }
//
//  sealed trait HeadOrBranch[S <: Base, A, E] /* extends Branch */ {
...
//  }
//
//  private final class SetLeaf[S <: Base, A](private[HASkipList] val entries: Vector[A])
//    extends Leaf[S, A, A] {
//
...
//  }
//
//  private final class MapLeaf[S <: Base, A, B](private[HASkipList] val entries: Vector[(A, B)])
...
//  }
//
//  sealed trait Leaf[S <: Base, A, E] extends Node[S, A, E] {
...
//
//  object Branch {
//    private[HASkipList] def read[S <: Base, A, B](in: DataInput, access: S.Acc,
//                                                    isRight: Boolean)
//                                                   (implicit tx: S.Tx,
...
//  }
//
//  final class Branch[S <: Base, A, B](private[HASkipList] val keys : Vector[A],
...
//
//  object Set {
//    type Node  [S <: Base, A] = HASkipList.Node  [S, A, A]
//    type Branch[S <: Base, A] = HASkipList.Branch[S, A, A]
//    type Leaf  [S <: Base, A] = HASkipList.Leaf  [S, A, A]
//
...
//      */
//    def empty[S <: Base, A](implicit tx: S.Tx, ord: Ordering[S.Tx, A],
...
//      * @param   keySerializer  the serializer for the elements, in case a persistent STM is used.
//      */
//    def empty[S <: Base, A](minGap: Int = 2,
...
//    }
//
//    def read[S <: Base, A](S: S)(in: DataInput, access: S.Acc,
//                                 keyObserver: SkipList.KeyObserver[S.Tx, A] = SkipList.NoKeyObserver)
//                          (implicit tx: S.Tx, ordering: Ordering[S.Tx, A],
//                             keySerializer: Serializer[S.Tx, S.Acc, A]): HASkipList.Set[S, A] = {
//
//      val id      = S.readId(in, access)
...
//    }
//
//    def serializer[S <: Base, A](keyObserver: SkipList.KeyObserver[S.Tx, A] = SkipList.NoKeyObserver)
//                                  (implicit ordering: Ordering[S.Tx, A],
...
//  }
//
//  sealed trait Set[S <: Base, A] extends SkipList.Set[S, A] {
//    def top(implicit tx: S.Tx): Option[HASkipList.Set.Node[S, A]]
//  }
//
//  object Map {
//    type Node  [S <: Base, A, B] = HASkipList.Node  [S, A, (A, B)]
//    type Branch[S <: Base, A, B] = HASkipList.Branch[S, A, (A, B)]
//    type Leaf  [S <: Base, A, B] = HASkipList.Leaf  [S, A, (A, B)]
//
//    /** Creates a new empty skip list with default minimum gap parameter of `2` and no key observer.
//      * Type parameter `S` specifies the STM system to use. Type parameter `A`
//      * specifies the type of the keys stored in the list.
...
//      * @param   keySerializer      the serializer for the elements, in case a persistent STM is used.
//      */
//    def empty[S <: Base, A, B](implicit tx: S.Tx, ord: Ordering[S.Tx, A],
//                                 keySerializer: Serializer[S.Tx, S.Acc, A],
...
//      * @param   keySerializer  the serializer for the elements, in case a persistent STM is used.
//      */
//    def empty[S <: Base, A, B](minGap: Int = 2,
//                                 keyObserver: SkipList.KeyObserver[S.Tx, A] = SkipList.NoKeyObserver)
//                                (implicit tx: S.Tx, ord: Ordering[S.Tx, A],
//                                 keySerializer: Serializer[S.Tx, S.Acc, A],
//                                 valueSerializer: Serializer[S.Tx, S.Acc, B]): HASkipList.Map[S, A, B] = {
...
//    }
//
//    def read[S <: Base, A, B](in: DataInput, access: S.Acc,
//                                keyObserver: SkipList.KeyObserver[S.Tx, A] = SkipList.NoKeyObserver)
//                               (implicit tx: S.Tx, ordering: Ordering[S.Tx, A],
//                                keySerializer: Serializer[S.Tx, S.Acc, A],
//                                valueSerializer: Serializer[S.Tx, S.Acc, B]): HASkipList.Map[S, A, B] = {
...
//    }
//
//    def serializer[S <: Base, A, B](keyObserver: SkipList.KeyObserver[S.Tx, A] = SkipList.NoKeyObserver)
//                                     (implicit ordering: Ordering[S.Tx, A],
//                                      keySerializer: Serializer[S.Tx, S.Acc, A],
...
//  }
//
//  sealed trait Map[S <: Base, /* @spec(KeySpec) */ A, /* @spec(ValueSpec) */ B] extends SkipList.Map[S, A, B] {
